...
export function initGame(options = {}) {
  ...
  function resetGame({ fromRemote = false } = {}) {
    ...
    state.overwriteRemaining[BLACK] = 3;
    state.overwriteRemaining[WHITE] = 3;
    resetDrawVotes();
  ...
  function endGame() {
    ...
    if (black > white) {
      overlay.textContent = `ゲーム終了: 黒の勝ち (${black} - ${white})`;
    } else if (white > black) {
      overlay.textContent = `ゲーム終了: 白の勝ち (${white} - ${black})`;
    } else {
