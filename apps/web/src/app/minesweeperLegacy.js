...
export function initMinesweeper(options = {}) {
  ...
  function normalizeCoopMissLimit(rawLimit) {
  ...
  function coopMissText() {
    ...
    const current = state.coopMisses
      .map((count, idx) => `${playerLabel(idx)}:M${Math.max(0, Number(count) || 0)}`)
      .join(" / ");
    return `${current} / L${state.coopMissLimit == null ? "∞" : String(state.coopMissLimit)}`;
  }
  ...
  function roomModePayload() {
    return {
      ...
      coopMissLimit: state.coopMissLimit,
    };
  }
  ...
  function applyRoomModePayload(payload = {}, { shouldEmit = false } = {}) {
    ...
    state.coopMissLimit = normalizeCoopMissLimit(payload.coopMissLimit ?? state.coopMissLimit);
    ...
    if (coopLivesSelectEl) coopLivesSelectEl.value = state.coopMissLimit == null ? "" : String(state.coopMissLimit);
  ...
  function updateHud() {
    ...
    if (coopLivesSelectEl) {
      coopLivesSelectEl.value = state.coopMissLimit == null ? "" : String(state.coopMissLimit);
      coopLivesSelectEl.disabled = state.playMode !== "coop" || !state.gameOver;
    }
    ...
    if (isRoomMode()) {
      ...
      if (coopLivesSelectEl) coopLivesSelectEl.disabled = lockedUi || state.playMode !== "coop";
        ...
        cellBtn.addEventListener("click", () => {
          ...
          if (targetCell.mine) {
            ...
            if (isCoopMode()) {
              ...
              const totalCoopMisses = state.coopMisses.reduce((sum, count) => sum + Math.max(0, Number(count) || 0), 0);
              if (state.coopMissLimit != null && totalCoopMisses >= state.coopMissLimit) {
                revealAllMines();
                state.gameOver = true;
                clearTimer();
  ...
  return {
    ...
    applySnapshot: (snapshot) => {
      ...
      state.coopMissLimit = normalizeCoopMissLimit(snapshot.coopMissLimit ?? state.coopMissLimit);
